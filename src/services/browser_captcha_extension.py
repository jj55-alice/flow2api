...
class ExtensionCaptchaService:
    ...
    def _require_image_ui_version(extension_version: str) -> None:
        try:
            ...
            version = ()
        if (version + (0, 0, 0))[:3] < (1, 3, 23):
            raise ExtensionCaptchaError(
                "Flow image generation needs Chrome extension 1.3.23+ for reliable progress reporting and result validation. Reload the updated Flow2API extension.",
