...
class FlowClient:
    ...
    def _resolve_generation_retry_budget(self, base_max_retries: int, error: Optional[Union[Exception, str]] = None) -> int:
        ...
        if "extension_route_busy" in error_lower or "remained busy" in error_lower:
            # Retrying immediately selects the same already-busy browser route.
            # Fail fast so the caller can submit a fresh request to another
            # account instead of growing a minutes-long in-process backlog.
            return 1
        if any(marker in error_lower for marker in [
            "flow browser submit returned no http response",
            "chrome extension flow submit timed out",
            "timed out waiting for the image generated by the flow ui",
        ]):
            # Every retry opens a fresh Flow tab. One recovery attempt is useful
            # for a lost execution context; more retries monopolize the account.
            return min(
                effective_max_retries,
                max(1, int(config.extension_transport_generation_retries or 1)),
            )
        return effective_max_retries
